...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('fasilitas-form').addEventListener('submit', async function(e) {
    ...
    request.onsuccess = async () => {
      // Sync ke Firebase (tambahan, pakai Compat)
      try {
        ...
        await firebase.database().ref('laporan_fasilitas').push({
          ...data,
          foto: fotoURL,
          timestamp: Date.now()
        });
      } catch (err) {
        console.log("Firebase sync gagal (data tetap lokal):", err);
      }
  ...
  document.getElementById('piket-form').addEventListener('submit', async function(e) {
    ...
    request.onsuccess = async () => {
      ...
      try {
        ...
        await firebase.database().ref('absensi_piket').push({
          ...data,
          foto: fotoURL,
          timestamp: Date.now()
        });
  ...
  async function loadUserReports() {
    ...
    // Load fasilitas reports (dari IndexedDB)
    ...
    // Load piket reports (dari IndexedDB)
