...
import { db, storage, ref, push, storageRef, uploadBytes, getDownloadURL } from './firebase-init.js';
let db;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Kirim juga ke Firebase (selain IndexedDB)
  try {
    const fileInput = document.getElementById('foto-fasilitas');
    let fotoURL = data.foto; // default dari preview

    if (fileInput.files[0]) {
      const file = fileInput.files[0];
      const storagePath = `fasilitas/${Date.now()}_${file.name}`;
      const fileRef = storageRef(storage, storagePath);
      await uploadBytes(fileRef, file);
      fotoURL = await getDownloadURL(fileRef);
    }

    await push(ref(db, 'laporan_fasilitas'), {
      ...data,
      foto: fotoURL,
      timestamp: Date.now()
    });
  } catch (err) {
    console.log("Firebase upload gagal (tapi tetap tersimpan lokal):", err);
  }
  ...
  try {
    ...
    await push(ref(db, 'absensi_piket'), {
      ...data,
      foto: fotoURL,
      timestamp: Date.now()
    });
